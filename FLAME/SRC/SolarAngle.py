...
def add_solar(good_spectra, solang):
    #
    # Add Solar_angle column to good_panels Dataframe based on 450nm
    # wavelength only (to save processing time).
    #
    solzen = good_spectra[good_spectra['Wavelength']==450].apply(solang, axis=1)
    
    #
    # Create Pandas Series with date_saved column, with the same length as "solzen".
    #
    times = good_spectra[good_spectra['Wavelength']==450].date_saved
